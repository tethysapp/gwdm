...
def rbf_field_generate(x_c, y_c, values, grid_x, grid_y):
    ...
    rbfi = sci_intrp.Rbf(
        x_c, y_c, values, epsilon=2, function="quintic", extrapolate=False
    )
...
def plot_np_data(
    nn_map, mesh_x, mesh_y, measurement, nn_plots, title, x_c, y_c, values
):
...
def fit_model_var(coords_df, x_c, y_c, values):
    ...
    bins_c = np.linspace(
        0, max_dist, bin_num
    )  # bin edges in variogram, bin_num of bins
